...
ROOT="${ROOT:-/storage/openpsi/users/zzy/rl-data-aug}"
RUN_NAME="polaris-qwen3-1.7b-aug"
EXPERIMENT_NAME="polaris_qwen3_1_7b_aug"
AUG_OPERATORS="${AUG_OPERATORS:-[\"synonym_substitution\"]}"
AUG_NUM_VARIANTS="${AUG_NUM_VARIANTS:-1}"
AUG_SEED="${AUG_SEED:-0}"
...
../.venv/bin/python -m verl.trainer.main_ppo \
  ...
  "data.augmentation.operators=${AUG_OPERATORS}" \
  data.augmentation.num_variants="${AUG_NUM_VARIANTS}" \
  data.augmentation.seed="${AUG_SEED}" \
  data.augmentation.source_id_field=extra_info.index \
  data.augmentation.semantic_similarity_threshold=0.5 \
  data.augmentation.rl_framework=verl \
  data.augmentation.dataset=polaris \
  data.augmentation.base_model=Qwen3-1.7B \
