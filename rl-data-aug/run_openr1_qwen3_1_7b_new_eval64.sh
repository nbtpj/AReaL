...
if [[ $# -ne 2 ]]; then
  echo "usage: $0 <eval-name> <val-data-file>" >&2
  exit 2
fi
...
export VERL_FILE_LOGGER_PATH="${ROOT}/${EVAL_NAME}_metrics.jsonl"
export PYTHONPATH="."
export VLLM_USE_V1=1
export VERL_RUN_TOKEN="${EVAL_NAME}-$(date +%s)"
...
../.venv/bin/python -m verl.trainer.main_ppo \
  ...
  trainer.default_local_dir="${ROOT}/ckpts/${EVAL_NAME}" \
  trainer.validation_data_dir="${ROOT}/logs/${EVAL_NAME}_validation" \
  trainer.rollout_data_dir="${ROOT}/logs/${EVAL_NAME}_rollout" \
