...
from verl.utils.dataset.augmentation import augment_rlhf_records, build_augmentation_config
...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.augmentation_config = build_augmentation_config(config)
    ...
    def _read_files_and_tokenize(self):
        ...
        self.dataframe = self.maybe_apply_augmentation(self.dataframe)
        self.dataframe = self.maybe_filter_out_long_prompts(self.dataframe)

    def maybe_apply_augmentation(self, dataframe: datasets.Dataset) -> datasets.Dataset:
        if not self.augmentation_config.enabled:
            return dataframe

        records = [dataframe[idx] for idx in range(len(dataframe))]
        augmented_records = augment_rlhf_records(
            records=records,
            prompt_key=self.prompt_key,
            config=self.augmentation_config,
        )
        print(f"augment dataset len: {len(augmented_records)}")
