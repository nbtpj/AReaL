...
export VERL_FILE_LOGGER_PATH="/storage/openpsi/users/zzy/rl-data-aug/vanilla_deepscaler_reward_metrics.jsonl"
export PYTHONPATH="."
export VLLM_USE_V1=1
export VERL_RUN_TOKEN="vanilla-deepscaler-reward-$(date +%s)"
...
../.venv/bin/python -m verl.trainer.main_ppo \
  ...
  reward.custom_reward_function.path=/storage/openpsi/users/zzy/rl-data-aug/verl/deepscaler/rewards/verl_correctness_reward.py \
  reward.custom_reward_function.name=compute_score \
  +reward.custom_reward_function.reward_kwargs.strip_comma_from_answer=True \
  'trainer.logger=["console","file"]' \
  trainer.project_name=rl_data_aug \
  trainer.experiment_name=vanilla_fix_reward_1_polaris_qwen3_1_7b \
  ...
  trainer.default_local_dir=/storage/openpsi/users/zzy/rl-data-aug/ckpts/vanilla-fix-reward-1 \
  trainer.validation_data_dir=/storage/openpsi/users/zzy/rl-data-aug/logs/vanilla-fix-reward-1_validation \
  trainer.rollout_data_dir=/storage/openpsi/users/zzy/rl-data-aug/logs/vanilla-fix-reward-1_rollout \
